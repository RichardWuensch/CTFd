...
def get_vms_in_group(group_name, base_vm_name):
    ...
    for line in output.strip().split('\n'):
        ...
        if f'groups="/{group_name}"' in vm_info:
            if 'VMState="poweroff"' in vm_info and not(vm_name.startswith(base_vm_name)):
...
def main():
    ...
    target_count = 10 #max number of existing Playground machines
    vms_in_group = get_vms_in_group(group_name, base_vm_name)
    while len(vms_in_group) < target_count:
        ...
        subprocess.run(['VBoxManage', 'modifyvm', clone_name, '--groups', f"/{group_name}"], stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT)
