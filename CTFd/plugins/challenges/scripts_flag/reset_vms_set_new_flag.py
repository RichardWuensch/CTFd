...
def change_flag_vm(vm_name, victims_connection, new_flag):
    subprocess.run(['python3', '/var/www/CTFd/CTFd/plugins/challenges/scripts_flag/changeFlag.py', vm_name, victims_connection, new_flag])

def change_flag(victims_connection, vm_name, flag):
    new_flag = subprocess.run(['python3', '/var/www/CTFd/CTFd/plugins/challenges/scripts_flag/generate_new_flag.py'], capture_output=True,
